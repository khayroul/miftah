...
function repeatLabel(value: number, t: (key: string) => string): string {
  return value === -1 ? t("repeatInfinite") : `${value}x`;
}
...
export function PageAudioRangeSettings({
  ...
  return (
    <>
      <div className="mt-3 grid gap-2 sm:grid-cols-3">
        ...
        <label className="text-xs text-stone-600 dark:text-stone-300">
          ...
          <select value={String(defaultRepeatCount)} onChange={(event) => onDefaultRepeatCountChange(normalizeRepeatValue(event.target.value))} className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-2 py-1.5 text-sm text-stone-900 dark:border-stone-600 dark:bg-stone-900 dark:text-stone-100">
            {REPEAT_OPTIONS.map((option) => <option key={option} value={option}>{repeatLabel(option, t)}</option>)}
          </select>
        </label>
        <label className="flex items-end gap-2 text-xs text-stone-600 dark:text-stone-300">
          ...
          <span>{t("loopSelectedRangeLabel")}</span>
        </label>
      </div>
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        {([[t("rangeStartLabel"), normalizedRangeStart, onRangeStartChange, "start"], [t("rangeEndLabel"), normalizedRangeEnd, onRangeEndChange, "end"]] as const).map(([label, value, onChange, key]) => (
      ...
      {rangeStatus ? <p className="mt-2 text-xs text-stone-500 dark:text-stone-400">{t("activeRangeLabel", { status: rangeStatus })}</p> : null}
      <details className="mt-3 rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 dark:border-stone-700 dark:bg-stone-800/45">
        <summary className="cursor-pointer text-xs font-medium uppercase tracking-wide text-stone-600 dark:text-stone-300">{t("perAyahRepeatLabel")}</summary>
        <div className="mt-3 space-y-3">
          <button type="button" onClick={onResetRangeRepeatOverrides} className="rounded-lg border border-stone-300 px-3 py-1.5 text-xs text-stone-700 transition hover:bg-stone-100 dark:border-stone-600 dark:text-stone-200 dark:hover:bg-stone-800">{t("resetRangeButton")}</button>
            ...
            {tracksInRange.map((track) => (
              <label key={`repeat-${track.key}`} className="text-xs text-stone-600 dark:text-stone-300">
                ...
                <select value={String(repeatByTrack[track.key] ?? defaultRepeatCount)} onChange={(event) => onTrackRepeatCountChange(track.key, normalizeRepeatValue(event.target.value))} className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-2 py-1.5 text-sm text-stone-900 dark:border-stone-600 dark:bg-stone-900 dark:text-stone-100">
                  {REPEAT_OPTIONS.map((option) => <option key={`${track.key}-${option}`} value={option}>{repeatLabel(option, t)}</option>)}
                </select>
              </label>
            ))}
          </div>
