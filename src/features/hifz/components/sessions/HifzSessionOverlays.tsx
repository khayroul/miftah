      ...
      <div className="flex items-center justify-between rounded-xl border border-stone-200 bg-white px-4 py-3 shadow-sm dark:border-stone-700 dark:bg-stone-900">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm text-stone-600 dark:text-stone-300">
            {phase === "playing" ? "Mendengar..." : phase === "reciting" ? "Sambung bacaan..." : "Sedia..."}
          </p>
        </div>
        {phase === "reciting" ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-xl border border-stone-200 bg-white px-4 py-2 shadow-sm dark:border-stone-700 dark:bg-stone-900">
            <div className="flex items-center justify-between text-xs text-stone-500 dark:text-stone-400">
              <span>{unveilState.revealedUpTo + 1} / {unveilState.totalWords} patah</span>
              <span>{progress}%</span>
            </div>
          ...
          <button type="button" onClick={onExit} className="text-center text-xs text-stone-400 underline-offset-2 hover:underline dark:text-stone-500">Keluar</button>
        </div>
      </div>
    </div>
  );
