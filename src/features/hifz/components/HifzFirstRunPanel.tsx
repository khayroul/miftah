...
export function HifzFirstRunPanel({
  ...
  const t = useTranslations("hifz.firstRun");
  return (
    <div className="rounded-3xl border border-stone-200/80 bg-white/85 p-6 shadow-sm backdrop-blur-sm dark:border-stone-700/50 dark:bg-stone-900/70 sm:p-8">
      ...
      {entryPath === "fresh" ? (
        <div className="mt-6 rounded-2xl border border-amber-200/80 bg-amber-50/80 p-5 dark:border-amber-700/35 dark:bg-amber-900/20">
          ...
          >
            {loading === "memorize" ? t("freshPreparing") : t("freshCta")}
          </button>
        </div>
      ) : null}
      ...
      {entryPath === "import" ? (
        <PageAction
          accent="indigo"
          buttonLabel={importing ? t("importCtaPending") : t("importCta")}
    ...
    <div className={`mt-6 rounded-2xl border p-5 ${styles}`}>
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex items-center gap-2">
          <label htmlFor={props.id} className={`text-sm font-medium ${labelStyles}`}>{t("pageInputLabel")}</label>
          <input
            id={props.id}
            type="number"
            min={1}
          ...
          />
          <span className={`text-sm ${suffixStyles}`}>{t("pageSuffix", { total: 604 })}</span>
        </div>
        <button
          type="button"
