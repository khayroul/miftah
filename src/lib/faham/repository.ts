...
export interface ReadPageVocabPreviewItem {
  ayahId: number;
  ayahNumber: number;
  occurrenceCount: number;
  pageNumber: number | null;
  position: number;
  status: "new" | "seen" | "learning" | "review" | "mastered";
  surahId: number;
...
export async function getReadPageVocabPreview(input: {
  ayahIds: number[];
  limit?: number;
  userId?: string | null;
  wordLimit: number;
}): Promise<ReadPageVocabPreviewItem[]> {
  const uniqueAyahIds = Array.from(
    new Set(input.ayahIds.filter((ayahId) => Number.isInteger(ayahId) && ayahId > 0)),
  );
  if (uniqueAyahIds.length === 0) {
    return [];
  }

  const topWordIds = await getTopFahamWordIds(input.wordLimit);
  if (topWordIds.length === 0) {
    return [];
  }

  type ReadPageWordRow = WordOccurrenceJoinRow & {
    ayat: AyahLite | AyahLite[] | null;
  };
  ...
  const { data, error } = await supabaseServer
    .from("word_occurrences")
    .select(
      "ayah_id, page_number, position, word_id, words!inner(id, text_uthmani, text_simple, translation_bm, translation_en, transliteration, root, lemma, pos, frequency), ayat!inner(surah_id, ayah_number)",
    )
    .in("ayah_id", uniqueAyahIds)
    .in("word_id", topWordIds)
    .order("ayah_id", { ascending: true })
  ...
  const wordsOnPage = new Map<number, {
    ayahId: number;
    ayahNumber: number;
    occurrenceCount: number;
    pageNumber: number | null;
  ...
  for (const row of (data ?? []) as ReadPageWordRow[]) {
    const word = firstRelation(row.words);
    const ayah = firstRelation(row.ayat);
    if (!word || !ayah) {
      continue;
    }
    if (!word.translation_bm && !word.translation_en) {
    ...
    const current = wordsOnPage.get(row.word_id);
    if (current) {
      current.occurrenceCount += 1;
      if (row.position < current.position) {
        current.position = row.position;
        current.ayahId = row.ayah_id;
        current.ayahNumber = ayah.ayah_number;
        current.surahId = ayah.surah_id;
        current.pageNumber = row.page_number;
      }
      continue;
    }

    wordsOnPage.set(row.word_id, {
      ayahId: row.ayah_id,
      ...
      translationBm: word.translation_bm ?? word.translation_en,
  ...
  const previewWords = Array.from(wordsOnPage.values());
  if (previewWords.length === 0) {
    return [];
  }

  const wordIds = previewWords.map((item) => item.wordId);
  const progressByWordId = new Map<number, VocabProgress>();
  const exposureWordIds = new Set<number>();
  const now = Date.now();

  if (input.userId) {
    const [{ data: progressData, error: progressError }, { data: exposureData, error: exposureError }] =
      await Promise.all([
        supabaseServer
          .from("vocab_progress")
          .select("*")
    ...
    for (const row of (progressData ?? []) as VocabProgress[]) {
      progressByWordId.set(row.word_id, row);
    }
    for (const row of (exposureData ?? []) as Array<{ word_id: number }>) {
      exposureWordIds.add(row.word_id);
    }
  }

  const ranked = previewWords
    .map((item) => {
      const progress = progressByWordId.get(item.wordId) ?? null;
      const hasExposure = exposureWordIds.has(item.wordId);
      let status: ReadPageVocabPreviewItem["status"] = "new";
      let statusRank = 2;
      ...
      if (progress?.is_mastered) {
        ...
        progress &&
        (progress.needs_reinforcement || new Date(progress.due).getTime() <= now)
      ) {
        status = "review";
        statusRank = 0;
      } else if (progress) {
  ...
  return ranked.slice(0, input.limit ?? 6).map((item) => ({
    ayahId: item.ayahId,
    ...
    translationBm: item.translationBm,
