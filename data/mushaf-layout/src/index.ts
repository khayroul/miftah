...
async function generateLines(pageData: any, page: number) {
    ...
    // Fallback for short pages where source words start at verse 1 but the
    // surah-header/basmala lines were dropped by line-count heuristics.
    if (lines.length <= 12) {
        const firstVerse = pageData.verses[0];
        const hasHeader = lines.some((bucket) => (bucket as any[])[0]?.type === "surah-header");
        const hasBasmala = lines.some((bucket) => (bucket as any[])[0]?.type === "basmala");

        if (firstVerse?.verseNumber === 1 && !hasHeader && !hasBasmala) {
            const surahNum = firstVerse.surahNumber;
            const name = await ArabicService.getSurahName(surahNum);
            if (surahNum !== 1 && surahNum !== 9) {
                lines.unshift([{ type: "basmala", qpcV2: QPCFontProvider.getBasmalaGlyph("2"), qpcV1: QPCFontProvider.getBasmalaGlyph("1") }]);
            }
            lines.unshift([{ type: "surah-header", text: name, surah: surahNum.toString().padStart(3, "0") }]);
        }
    }
