...
**Your immediate path:**
1. **Re-run two lanes that were stopped mid-flight** (zero committed work; tree is clean). Handoff §2 has the specs. Order: (a) the **understanding-coverage engine** (north-star foundation: cached coverage%+tier engine + the frequency data-quality verification); (b) the **double-auth fix** (delicate — must STOP-and-defer to Lane A if not provably safe; this is AUTH).
