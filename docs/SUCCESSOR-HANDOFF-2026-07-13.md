...
## 2. TWO LANES STOPPED INCOMPLETE AT HANDOFF — RE-RUN THESE FRESH
Both were killed mid-flight with ZERO committed work (worktrees + branches already removed; tree is clean, only `main`). Re-dispatch them fresh:
- **Understanding-coverage engine** (RE-RUN FIRST — north-star foundation): build the cached coverage%+tier ENGINE (no UI) + the §0 data-quality verification (is 96,219 defensible? is `is_mastered` the right "understood" bar?). Place it to survive the restructure (`src/data/` repo layer / a `features/understanding` or `src/lib/faham/coverage.ts` module). API: `getUnderstandingCoverage(userId)` + `getCoverageTiers(userId)` for tiers 10/100/300/500/1000/2000/5000. MUST cache the global denominator + tier boundaries (don't re-aggregate 21,977 words per request); per-user = one indexed query on the now-live `idx_words_frequency`/`idx_vocab_progress_word_id`. Fixture-based tests. (The stopped worker had finished mapping the data model — you're re-deriving cheap analysis, not losing built code.)
- **Double-auth fix** (LOWER priority, delicate): eliminate the redundant per-request `auth.getUser()` (middleware + route both call GoTrue). HARD GUARDRAIL: if the safe fix isn't provably safe (anti-spoof header strip, or local JWT verify), STOP and defer to Phase-2 Lane A — this is AUTH, a wrong change = auth bypass. Also the middleware rate-limit/token-rotation-on-429 hazard lives here. Deferrable — don't gamble to save one round trip.
